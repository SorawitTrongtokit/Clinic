...
import { User, FileText, Printer, Clock, ArrowLeft, AlertCircle, Phone, MapPin, Edit, Activity, LayoutList, Copy, Check, Trash } from 'lucide-react';
...
export default function PatientDetailView({ id }: PatientDetailViewProps) {
    ...
    const handleStartVisit = () => {
        ...
        // We defer creation until the end of the wizard to avoid empty/null visits
        router.push(`/visit/new?patient_id=${patient.id}`);
    };

    const handleDeleteVisit = async (visitId: string) => {
        if (!confirm('คุณต้องการลบประวัติการรักษานี้ใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้')) return;
                                            ...
                                            <div className="divide-y divide-slate-100 -mt-2">
                                                {visits.map((visit) => (
                                                    <div key={visit.id} className="py-4 hover:bg-slate-50/50 transition-colors group rounded-lg px-2 -mx-2">
                                                        ...
                                                        <div className="flex justify-end gap-2 mt-3 pt-3 border-t border-slate-50/50">
                                                            <Button
                                                                variant="outline"
                                                                size="sm"
                                                                className="gap-1 h-7 text-xs border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300"
                                                                onClick={() => handleDeleteVisit(visit.id)}
                                                            >
                                                                <Trash className="w-3 h-3" /> ลบรายการ
                                                            </Button>
